...
void MostrarHistogramas(Mat img){
	...
	//HISOGRAMA DE LAS IMAGENES EN LOS 3 CANALES PARA BGR Y HSV
...
Mat Balance_Colores(Mat in,float percent) {
...
//La imagen debe estar en escala de grises. Filtros pasa alto y pasa bajo en frecuencia.
//altobajo = 0 -> Pasa bajos, altobajo = 1 -> Pasa altos.
...
//La imagen debe estar en tono de grises. Bueno para ruido gaussiano, malo para impulsivo (sal y pimienta)
...
Mat filter_imaginario(cv::Mat image, cv::Mat filtro_magnitud){
	//se asume imÃƒÂ¡genes de 32F y un canal, con tamaÃƒÂ±o ÃƒÂ³ptim
	cv::Mat transformada;
	
	//como la fase es 0 la conversiÃƒÂ³n de polar a cartesiano es directa (magnitud->x, fase->y)
	//	cv::Mat x[2];
...
///%%%%%%%%%%%%%%%%%%%%%%%%%% ESTA NO ANDA BIEN %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
///%%%%%%%%%%% FACU ENCONTRO UNA FUNCION QUE LO HACE BIEN %%%%%%%%%%%%%%%%%%%%%%
...
void Hough(cv::Mat im,cv::Mat &cdst,cv::Mat &dst,double threshold,int &cx,int &cy,bool vertical=0,bool horizontal=0){
	...
	for( size_t i = 0; i < lines.size(); i++ ){
		...
		if(( theta>(CV_PI/180)*170 || theta<(CV_PI/180)*10) and vertical){ 
...
//Luego a la funcion debemos llamarla asi: 
...
Mat CrecimientoRegiones(Mat img,int x,int y){ //Recibe la imagen a segmentar y un (x,y) donde se planta la semilla.
	...
	//Parametros de floodFill
	//1° :Imagen a segmentar.
	//2° :Punto x,y donde se planta la semilla.
	//3° :Nuevo valor del dominio de los pixeles pintados (valores que toman regiones cercanas a la semilla).
	//4° :Opcional --> NULL
	//5° :Minima diferencia de brillo / color entre el pixel observado y uno de sus vecinos (tolerancia inferior)
	//6° :Maxima diferencia de brillo / color entre el pixel observado y uno de sus vecinos (tolerancia superior)
...
//Media y desvio de un histograma, lo aplico en el canal HSV[0] para calcular 
//una mascara binaria en base el histograma, de la siguiente forma:
...
//Mat img = imread("Imagenes/ejercicio2.tif",CV_LOAD_IMAGE_GRAYSCALE);
//img.convertTo(img,CV_32F,1./255);
...
//1° Forma del elemento: MORPH_RECT (rectangular), MORPH_ELLIPSE (elipse), MORPH_CROSS (cruz), CV_SHAPE_CUSTOM (personalizado)
//2° Size (n*n) tamaño del EE.
