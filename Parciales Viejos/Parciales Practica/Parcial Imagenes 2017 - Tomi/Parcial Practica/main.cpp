...
void histogramaHSV(vector<Mat>hsv){
	...
	Averiguar("Histo B",canvas_HSV_H);
	Averiguar("Histo G",canvas_HSV_S);
	Averiguar("Histo R",canvas_HSV_V);
...
void parcialBrujula(Mat imgInput){
	...
	///Hago roi para ver los componentes BGR del color de la brujula
	...
	///Calculo componentes BGR
	vector<Mat>BGR;
	split(roi,BGR);
	
	///Hago histogramas para ver minimos y maximo de BGR
	//Haciendo click en la zona de interes averiguo las posiciones
	histogramaHSV(BGR); //Funcion definida mas arriba
